...
public class S3FileSystemProvider extends FileSystemProvider {
    ...
    @Override
    public OutputStream newOutputStream(final Path path, final OpenOption... opts) throws IOException {
        LOGGER.debug("New output stream. path:{}, options:{}", path, opts);
        ...
        final Set<OpenOption> options = new LinkedHashSet<>(Arrays.asList(opts));
        final boolean multipartEnabled = Boolean.parseBoolean(properties.getProperty(MULTIPART_UPLOAD_ENABLED, "false"));

        // cannot handle APPEND here -> use super.newOutputStream() implementation
        if (options.contains(StandardOpenOption.APPEND) || ! multipartEnabled) {
            return super.newOutputStream(path, opts);
        }
        ...
        return createMultipartUploadOutputStream(s3Path, options);
    }
